...
class SAM_masks_with_EgoVIDEO(ABC):
    def __init__(
        self,
        cfg: dict,
        device: torch.device,
    ...
    def load(self):
        SAM_feat_maps = {}
        SAM_video_IDs = []
        for video_path in self.image_paths:
            video_ID = video_path.split("/")[-1].split(".")[0].split("_")[-1]
            SAM_video_IDs.append(video_ID)
            filename = os.path.join(self.cache_path, video_ID)
            #SAM_seg_maps.append(np.load(filename + "_s.npy")) #SEG MAP is (4, 245, 456)
            SAM_feat_maps[video_ID] = {}
            SAM_feat_maps[video_ID]['feat_map'] = np.load(filename + "_f.npy") #FEATURE MAP IS (N, 512)
            SAM_feat_maps[video_ID]['seg_map'] = np.load(filename + "_s.npy") #SEG MAP IS (4, 245, 456)
            #N goes from -1 to N. If it is -1, it is the border and they should be ignored in the loss function
        self.SAM_video_ids = SAM_video_IDs
        self.SAM_video_feat_seg_maps = SAM_feat_maps
        #SAM_feat_maps = torch.stack([torch.tensor(x) for x in SAM_feat_maps])
        #print(SAM_feat_maps.shape, 'the shape of the feat maps')
    
    def __call__(self, img_points):
        video_IDs = [self.SAM_video_ids[int(x)] for x in img_points[:, 0]]
        ...
        for i, ID in enumerate(video_IDs):
            feat_map = self.SAM_video_feat_seg_maps[ID]['feat_map']
            seg_map = self.SAM_video_feat_seg_maps[ID]['seg_map']
