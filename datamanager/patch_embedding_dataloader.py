...
class PatchEmbeddingDataloader(FeatureDataloader):
    ...
    def load(self):
        ...
        print("Loaded data shape: ", self.data.shape)
    ...
    def create(self, image_list, images_path):
        ...
        """print('We proceed to PCA reduction of the CLIP embeddings to', self.pca_components, 'PCA components')
        #Apply PCA reduction
        pca = PCA(n_components=self.pca_components, copy=False)
        N, H, W, C = self.data.shape
        all_features = self.data.reshape(-1, C).numpy()
        X = pca.fit_transform(all_features)
        self.data = torch.Tensor(X).view(N, H, W, self.pca_components).half()
