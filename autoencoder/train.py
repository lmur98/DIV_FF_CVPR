...
if __name__ == '__main__':
    print("Start training de oooootra secuencia")
    ...
    parser.add_argument('--encoder_dims',
                    nargs = '+',
                    type=int,
                    default=[256, 128] #, 32, 3],
                    )
    parser.add_argument('--decoder_dims',
                    nargs = '+',
                    type=int,
                    default=[196, 256, 256, 512], #[16, 32, 
                    )
    parser.add_argument('--dataset_name', type=str, required=True)
    args = parser.parse_args()
    dataset_path = args.dataset_path
    num_epochs = args.num_epochs
    data_dir = f"{dataset_path}/SAM2bbox_CLIP_features"
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for idx, feature in enumerate(train_loader):
            data = feature.to("cuda:0").float()
            outputs_dim3 = model.encode(data) # (N, 64)
