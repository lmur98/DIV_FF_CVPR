...
class NeuralDiff(nn.Module):
    def __init__(
        ...
        """network_1 = tcnn.Network(
            n_input_dims=in_channels_xyz,
            n_output_dims=W,
            network_config={
                "otype": "CutlassMLP",
                "activation": "ReLU",
        ...
        network_2 = tcnn.Network(
            n_input_dims=in_channels_xyz+W,
            n_output_dims=W,
            network_config={
                "otype": "CutlassMLP",
                "activation": "ReLU",
        ...
        self.xyz_encoding = nn.Sequential(network_1, network_2)
        # self.xyz_encoding = tcnn.Network(
        #     n_input_dims=in_channels_xyz,
        #     n_output_dims=W,
        #     network_config={
        #         "otype": "CutlassMLP",
        ...
        # self.static_features = nn.Sequential(nn.Linear(W // 2, 64), nn.Tanh())           
        ...
        if self.encode_dynamic:
            ...
            # self.transient_features = nn.Sequential(nn.Linear(W // 2, 64), nn.Tanh())
            ...
            # self.person_features = nn.Sequential(nn.Linear(W // 2, 64), nn.Tanh())
    ...
    def forward(self, x, sigma_only=False, output_dynamic=True):
        ...
        # xyz_ = self.xyz_encoding[0](xyz_)
        #xyz_ = torch.cat([input_xyz, xyz_], 1)
        #xyz_ = self.xyz_encoding[1](xyz_)
        #xyz_ = xyz_.to(torch.float32)
        # xyz_ = self.xyz_encoding(xyz_)
        # xyz_ = xyz_.to(torch.float32)
        ...
        # static_features = self.static_features(dir_encoding)
        # static = torch.cat([static_rgb, static_sigma, static_features], 1) # (B, 3 + 1 + 64)
        static = torch.cat([static_rgb, static_sigma], 1) # (B, 3 + 1)
        ...
        # transient_features = self.transient_features(transient_encoding) # (B, 64)
        # transient = torch.cat(
        #     [transient_rgb, transient_sigma, transient_beta, transient_features], 1
        # )  # (B, 5 + 64)
        transient = torch.cat(
        ...
        # person_features = self.person_features(person_encoding) # (B, 64)
        # person = torch.cat([person_rgb, person_sigma, person_beta, person_features], 1)  # (B, 5 + 64)
        person = torch.cat([person_rgb, person_sigma, person_beta], 1)  # (B, 5)

        # final outputs contain static, transient and person model
        return torch.cat([static, transient, person, # (B, 3 + 1 + 3 + 1 + 3 + 1)
                          dir_encoding, transient_encoding, person_encoding], 1)  # (B, 68 + 69 + 69) = (B, 206)
