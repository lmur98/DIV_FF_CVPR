...
def render_rays(
    ...
    **kwargs,
    ...
    def lerf_inference(results, models, encoding, scales, weights, test_time=False, preset_scales=None, **kwargs):
        """
        ...
        dino_out_list = []
        for p in parts:
            clip_out, dino_out = models[p + '_lerf'](encoding[p], scales[p])
            ...
            dino_out = torch.sum(weights[p] * dino_out, dim=-2) #MOD: previously it was detach()
            dino_out_list.append(dino_out)

        results["clip"] = torch.nn.functional.normalize(sum(clip_out_list), dim = -1)
        results["dino"] = torch.nn.functional.normalize(sum(dino_out_list), dim = -1)

        if test_time:
            with torch.no_grad(): 
                max_across, best_scales = get_max_across(models, image_encoder, encoding, weights, scales['static'].shape, preset_scales)
                results["raw_relevancy"] = max_across # [N_positives, N_Rays, 1] 
                results["best_scales"] = best_scales # [N_positives]
    ...
    if N_importance > 0:
        ...
        if hp.use_clip:
            ...
            static_xyz_lerf = torch.gather(xyz_fine, -2, best_ids.expand(*best_ids.shape[:-1], 3)) # Select from xyz coarse the best ray samples according to the best weights ids
            static_encoding_lerf = torch.gather(results["static_encoding"], -2, best_ids.expand(*best_ids.shape[:-1], 128))
            # Normalize scales over ray distance and image height for training or override scales if inference
            if test_time:
                placeholder = torch.empty(clip_scales.shape[0], N_lerf_samples, 1).to(torch.device("cuda:0"))
                static_clip_scales = torch.ones_like(placeholder)
            else:
                with torch.no_grad():
                    dist = static_xyz_lerf.norm(dim=-1, keepdim=True)
                    static_clip_scales = clip_scales[..., None] # [N_Rays, 1, 1]
                static_clip_scales = static_clip_scales * img_h * (dist / intrinsics_fy) # [N_Rays, N_lerf_samples, 1]
            ...
            transient_xyz_lerf = torch.gather(xyz_fine, -2, best_ids.expand(*best_ids.shape[:-1], 3)) # Select from xyz coarse the best ray samples according to the best weights ids
            transient_encoding_lerf = torch.gather(results["transient_encoding"], -2, best_ids.expand(*best_ids.shape[:-1], 128))
            if test_time:
                placeholder = torch.empty(clip_scales.shape[0], N_lerf_samples, 1).to(torch.device("cuda:0"))
                transient_clip_scales = torch.ones_like(placeholder)
            else:
                with torch.no_grad():
                    dist = transient_xyz_lerf.norm(dim=-1, keepdim=True)
                    transient_clip_scales = clip_scales[..., None]
                transient_clip_scales = transient_clip_scales * img_h * (dist / intrinsics_fy)
            
            #PERSON MODEL
            person_weights, best_ids = torch.topk(rearrange(results["person_weights"], "n1 n2 -> n1 n2 1"), N_lerf_samples, dim=-2, sorted=False)
            person_xyz_lerf = torch.gather(xyz_fine, -2, best_ids.expand(*best_ids.shape[:-1], 3))
            person_encoding_lerf = torch.gather(results["person_encoding"], -2, best_ids.expand(*best_ids.shape[:-1], 128))
            if test_time:
                placeholder = torch.empty(clip_scales.shape[0], N_lerf_samples, 1).to(torch.device("cuda:0"))
                person_clip_scales = torch.ones_like(placeholder)
            else:
                with torch.no_grad():
                    dist = person_xyz_lerf.norm(dim=-1, keepdim=True)
                    person_clip_scales = clip_scales[..., None]
                person_clip_scales = person_clip_scales * img_h * (dist / intrinsics_fy)

            encoding_lerf = {'static': static_encoding_lerf, 'transient': transient_encoding_lerf, 'person': person_encoding_lerf}
            clip_scales = {'static': static_clip_scales, 'transient': transient_clip_scales, 'person': person_clip_scales}
            ...
            if test_time:
                ...
                lerf_inference(
                    ...
                    weights,
                    test_time,
                    preset_scales=results["best_scales"]
                )        
