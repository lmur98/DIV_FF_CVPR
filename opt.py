...
def get_opts(vid=None, root="data/EPIC-Diff"):
    ...
    parser.add_argument(
        "--ckpt_path_pretrained",
        type=str,
        default=None,
        help="Pretrained checkpoint path to load for feature distillation (DINO+CLIP).",
    )
    ...
    parser.add_argument(
        "--lerf_model_width", type=int, default=256, help="Width of lerf model (units per layer)."
    )    
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Freeze NeuralDiff layers to train CLIP standalone",
    )
